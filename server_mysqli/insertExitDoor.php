...
if ($stmt = mysqli_prepare($link, $query)) {
    mysqli_stmt_bind_param($stmt, "ssssss", $ed, $exitDoorXpos, $exitDoorYpos,$context,$title, $empty);
...
if ($stmt = mysqli_prepare($link, $query)) {
    mysqli_stmt_bind_param($stmt, "sssss", $componentLastItemID,$thisDoorQuestion, $TRUE, $contextExited, $thisAnswerQuery);
    mysqli_stmt_execute($stmt);
    if(mysqli_stmt_affected_rows($stmt)==0){
