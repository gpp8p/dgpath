...
/*
    if ($stmt = mysqli_prepare($link, $query)) {
        ...
        if(mysqli_stmt_affected_rows($stmt)==0){
            header('HTTP/1.0 400 Nothing loaded - load all components in a context');
            exit;
        }else{
            mysqli_stmt_bind_result($stmt, $component_title, $componentXpos, $componentYpos, $componentType,$component_id,$componentContent );
            $dataFound = false;
