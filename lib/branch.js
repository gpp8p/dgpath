...
function branch(loadedComponent) {
    ...
    var branchEntryScreen = function(t,x,y,n, popName){
        ...
        $("#createBranch").on('click', function(){
            ...
            createBranchComponent(componentX, componentY, $("#branchingComponentTitleValue").val(), ex);
        });
    ...
    var createBranchComponent = function(posX, posY, title, doCreateExits){
        ...
        myData = { xpos:posX, ypos:posY, type:"branch", content: "", title: title, context: thisContext, events: sevts, showSub: "false", elementId: elementId, reloadContext:thisContext};
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                if(doCreateExits){
                    createExits(posX, posY,createdComponentId);
                }else{
                    $("#"+popName).html("");
                    $("#"+popName).hide();
                    var thisContextId = $("#componentContext").val();
    ...
    var createExits = function(currentX, currentY, branchComponentId){
        ...
        var exit1Y = currentY+100;
        var exit2X = currentX+200;
        var exit2Y = currentY+200;
    ...
    var createExit2 = function(x,y){
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                createPassFailLinks(passDoorId, failDoorId, branchComponentId);
            },
            error: function(err) {
                alert(err.toString());
                if (err.status == 200) {
    ...
    var createPassFailLinks=function(passId, failId, branchId){

    }

    var errorDialog = function(errMsg){
