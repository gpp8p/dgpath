...
function branch(loadedComponent) {
    ...
    this.entry = function (thisComponent) {
        var returnBranchUserEvents = function() {
            var thisUserEventsArray = [];
 //           thisUserEventsArray.push(new userEvent(thisComponent.id, "", branchEntered, thisComponent.elementId));
    ...
    var branchEditScreen = function(thisComponent, popName, connections) {
        ...
        var popName = popName;
        var content = thisComponent.content;
        var contentObject = JSON.parse(content);
        var selectedOption = contentObject.passFailPoint;
        var exitsGenerated = contentObject.generateExits;
        ...
        $("#branchingComponentTitleLabel").html($.t("branch.branchingComponentTitleLabel"));
        $("#noComponentTitle").hide();
        $("#buildExitOption").hide();
        $("#updateBranch").hide();
        if (exitsGenerated) {
            $("#branchingExitOptionLabel").html($.t("branch.branchingExitOptionEditLabel"));
            $("#updateBranch").prop('value',$.t("branch.updateBranch"));
            $("#updateBranch").show();
            $("#updateBranch").on('click', function(){
                var thisBranchContent = new branchContent(true, $("#branchThreshold option:selected").val());
                var thisBranchContentEncoded = JSON.stringify(thisBranchContent);
                updatePassFailLinks(componentId, $("#branchThreshold option:selected").val(), popName, thisBranchContentEncoded)
            });
        }
        ...
        $("#branchThreshold").append($('<option>', { selected: false, value :"10" }).text($.t("branch.branchat10")));
        $("#branchThreshold").append($('<option>', { selected: false, value :"05" }).text($.t("branch.branchat05")));
        var selectedOptionSpec = "branchThreshold option[value='"+selectedOption+"']";
        $("#"+selectedOptionSpec).prop('selected', true);
        $("#createBranch").hide();
        $("#removeBranch").prop('value',$.t("branch.removeBranch"));
    ...
    var getBranchDialogHtml = function(){
        ...
        strVar += "		<input type=\"button\" id=\"updateBranch\"\/>";
        strVar += "	<\/div>";
    ...
    var updatePassFailLinks=function(branchId, passFailPoint, popName, newContent){
        var popName = popName;
        var myData = {passFailPoint: $("#branchThreshold option:selected").val(), componentId: branchId, content: newContent };
        var thisTransaction = urlBase+edit_branch+"?"+debug;
        $.ajax({
            type: "POST",
            url: thisTransaction,
            data: myData,
    ...
    var updatePassFail = function(passFailThreshold){
