...
var cloneThis = function(pn, cloneObj, direction){
    ...
    var getConfirmCopy = function(popName, objectTitle, ObjectId, ObjectType, selectedTitle, targetFolder, event){
        alert("ok");
    }
    ...
    var getConfirmClone = function(popName, objectTitle, ObjectId, ObjectType, selectedTitle, targetFolder, event){
        ...
        var getConfirmCloneHtml = function(){
            ...
            strVar += "<div id=\"newContextName\" class=\"ruSureCss\"><span id=\"newContextNameLabel\"><\/span><span><input id=\"newContextNameValue\" type=\"text\" size=\"40\" \/><\/span></div>";
            strVar += "<div><input type=\"button\" id=\"cloneYes\" \/><input type=\"button\" id=\"cloneNo\" \/></div>";
            return strVar;
        }
        ...
        $("#cloneYes").on('click', function(){
            ...
            var myData = {subcontext:subContextId, target: thisTargetFolder, newContextName: thisNameContextName};
            $.ajax({
                type: "POST",
                url: thisTransaction,
                data: myData,
        ...
        $.each(seIds, function(key, value){
            ...
            if(cloneDirection==0) {
                ...
                if(thisFolderIds[3]=="subcontext"){
                    ...
                    $("#" + selectId).on('click', function (event) {
                        event.stopPropagation();
                        var copyTarget = $("#saveContext").val();

                        getConfirmCopy(popName, objectTitle, objectId, objectType, thisFolderIds, targetFolder, event);
                    });
                }
            }
        });
