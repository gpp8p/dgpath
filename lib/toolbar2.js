...
function setupThisProject(contextId){
    ...
    var thisContextId = contextId;
    inContextNow = thisContextId;
    var thisFunction = "authorSelect";
    var myData = {contextId:thisContextId, logFunction: thisFunction};
    var thisTransaction = urlBase+log_user_event+"?"+debug;
    $.ajax({
        type: "POST",
        url: thisTransaction,
        data: myData,
...
//                userEventsPresentConfirmDelete(this.componentId);
                $("#alertPopup").html(alertPopupHtml("User events already loogged for this component. Do you really want to delete: " + this.componentTitle + " ?"));
                $("#alertPopup").css('font-size', '18px');
                $("#alertPopup").show();
...
var cloneThis = function(pn, cloneObj, direction){
    ...
    var getConfirmCopy = function(popName, copySource, copyTarget, insertLocationX, insertLocationY, description, project, event){
        ...
        $("#cloneYes").on('click', function(event) {
            ckInstance = findCkInstanceByName("subcontextDescription");
            var contextDescription = ckInstance.getData();
            var contextLabel = $("#newContextNameValue").val();
            var myData = {copyTarget: thisCopyTarget, copySource: thisCopySource, componentX: locX, componentY: locY, description: contextDescription, thisProject:thisProject, thisContextLabel:contextLabel};
            thisTransaction = urlBase + copy_context_from + "?" + debug;
    ...
    var copyFromLibrary = function(popName, copySource, copyTarget, insertLocationX, insertLocationY, event){
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                var thisComponentProject = thisComponent.project;
                getConfirmCopy(thisPopName,thisCopySource,thisCopyTarget, thisInsertLocationX, thisInsertLocationY,thisComponentDescription, thisComponentProject);
            },
            error: function(err) {
                alert(err.toString());
                if (err.status == 200) {
...
var selectDashboardTab = function(){
    ...
    thisDashBoard.showDashBoardSkeleton();
